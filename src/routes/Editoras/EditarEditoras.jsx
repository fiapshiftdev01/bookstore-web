...
const EditarEditoras = () => {
    ...
    const apiEditoras = async () => {
      try {
        ...
        if (error.response?.status === 404) {
          setError("Editora Não encontrado");
        } else {
          setError("Erro ao buscar Editora");
        }
        setTimeout(() => navigate("/editoras"), 2000); // Redireciona após 2 segundos em caso de erro
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setError(error.response?.data?.error || "Error ao alterar editora");
    }
  };
    ...
    return (
      <div className="max-w-md mx-auto mt-10 p-6 bg-white rounded shadow">
        ...
        <p className="text-center mt-4">
          Redirecionando para lista de Editoras...
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-md mx-auto mt-10 p-6 bg-white rounded shadow">
      <h2 className="text-2xl font-bold mb-6">Editar Editora</h2>
